# chat_history_server MCP Server

SuperBuilder Chat History MCP Server
...
## Configuration
- **Server Name**: chat_history_server
- **Protocol**: stdio
- **Default Port**: N/A (stdio protocol)
- **Environment Variables**:
  - `MCP_CHAT_HISTORY_SERVER_PROTOCOL`: Override protocol (stdio, sse, http)
  - `MCP_CHAT_HISTORY_SERVER_PORT`: Override default port (only for sse/http protocols - current: 7905)
...
# The executable will be created in dist/chat_history_server-mcp-server.exe
```

### 4. Production Usage
```powershell
# Run the built executable (using stdio protocol)
.\dist\chat_history_server-mcp-server.exe start

# Check status
.\dist\chat_history_server-mcp-server.exe status --json

# Stop server
.\dist\chat_history_server-mcp-server.exe stop
```
...
async def my_custom_tool(input_param: str) -> str:
    ...
    return f"Processed: chat_history_server"
```
...
# Run specific test
python -m unittest test_server.TestChat_history_serverServer.test_server_version
```
...
Build output:
- `dist/chat_history_server-mcp-server.exe`: Standalone executable
...
Copy-Item .\dist\chat_history_server-mcp-server.exe C:\MyServers\

# Run on target system
C:\MyServers\chat_history_server-mcp-server.exe start
```
...
.\dist\chat_history_server-mcp-server.exe start                              # Start with configured protocol
.\dist\chat_history_server-mcp-server.exe start --protocol sse --port 8080    # Override to use SSE protocol with custom port
.\dist\chat_history_server-mcp-server.exe status --json
.\dist\chat_history_server-mcp-server.exe stop
```
...
├── chat_history_server_server.spec # PyInstaller specification
