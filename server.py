...
# Message templates
SERVER_MESSAGES = {
    'server_start': 'chat_history_server MCP server starting' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_started': 'chat_history_server MCP server started successfully' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_stopped': 'chat_history_server MCP server stopped',
    'server_start_fail': 'Failed to start chat_history_server MCP server',
    ...
    'status_running': 'chat_history_server MCP server is running',
    'status_stopped': 'chat_history_server MCP server is not running',
    'ping_success': 'Server is responding',
    'ping_fail': 'Server is not responding',
    'interrupt': 'Operation cancelled by user',
    'error': 'Error: {error}',
    'version': 'chat_history_server MCP Server v{version}',
}
...
def create_response(success, message, data=None, exit_code=0, current_port=None, current_protocol=None):
    ...
    response = {
        ...
        "protocol": current_protocol or chat_history_server_PROTOCOL,
    }
...
# Generated by McpServerGen.py on 2025-11-15 20:17:46
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
chat_history_server_PORT = int(os.getenv('MCP_CHAT_HISTORY_SERVER_PORT', 7905)) if 'stdio' != 'stdio' else None
chat_history_server_PROTOCOL = os.getenv('MCP_CHAT_HISTORY_SERVER_PROTOCOL', 'stdio')
...
async def get_chat_history(session_id: int) -> dict:
    ...
    Retrieves all previous chat history of a chat session given the session ID.
    
    Args:
        session_id: chat session ID
    
    Returns:
        dict: the conversation history of this chat session.
    """
    import json
...
def start_server(port=None, protocol=None):
    ...
    if port is None:
        port = chat_history_server_PORT
    if protocol is None:
        protocol = chat_history_server_PROTOCOL
    
    # For stdio protocol, we don't need a port
    if protocol == 'stdio':
        port = None
    
    if (protocol == 'stdio' and mcp is None) or (protocol != 'stdio' and port != chat_history_server_PORT):
        # Create fresh FastMCP instance with current port and protocol
        if protocol == 'stdio':
            mcp = FastMCP("chat_history_server")
        else:
            mcp = FastMCP("chat_history_server", port=port)
...
def stop_server(json_mode=False, port=None):
    ...
    if chat_history_server_PROTOCOL == 'stdio':
        port = None
                    ...
                    try:
                        ...
                        if proc.info['name'] and 'python' in proc.info['name'].lower():
                            ...
                            if (cmdline and 
                                ...
                                any('chat_history_server' in str(arg).lower() for arg in cmdline)):
...
def is_server_running():
    ...
    return _server_running and is_port_in_use(chat_history_server_PORT)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParserWithExceptions(
        description="Chat History Server",
...
Examples:
  ...
  python server.py version
    ...
    try:
        ...
        current_port = chat_history_server_PORT
        current_protocol = chat_history_server_PROTOCOL
        if args.port:
            current_port = args.port
            os.environ[f'MCP_CHAT_HISTORY_SERVER_PORT'] = str(current_port)
        if args.protocol:
            current_protocol = args.protocol
            os.environ[f'MCP_CHAT_HISTORY_SERVER_PROTOCOL'] = current_protocol
        ...
        if current_protocol == 'stdio':
            ...
            mcp = FastMCP("chat_history_server", port=current_port)
        ...
        current_port_safe = locals().get('current_port', chat_history_server_PORT)
        current_protocol_safe = locals().get('current_protocol', chat_history_server_PROTOCOL)
        response, exit_code = create_response(
            True, 
            format_message('interrupt'),
